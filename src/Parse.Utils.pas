...
type
  ...
  { TParseCallback }
  TParseCallback<T> = record
    Callback: T;
    UserData: Pointer;
    function IsAssigned(): Boolean;
  end;

  { TParseCaptureConsoleCallback }
  TParseCaptureConsoleCallback = reference to procedure(const ALine: string; const AUserData: Pointer);

  { TParseStatusCallback }
  TParseStatusCallback = reference to procedure(const AText: string; const AUserData: Pointer);

  { TParseVersionInfo }
  TParseVersionInfo = record
    Major: Word;
    Minor: Word;
    Patch: Word;
    Build: Word;
  ...
  public
    ...
    class procedure CaptureConsoleOutput(const ATitle: string; const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TParseCaptureConsoleCallback); static;
    class procedure CaptureZigConsolePTY(const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TParseCaptureConsoleCallback); static;
    ...
    class function  GetVersionInfo(out AVersionInfo: TParseVersionInfo; const AFilePath: string = ''): Boolean; static;
  ...
  { TParseCommandBuilder }
  TParseCommandBuilder = class(TParseBaseObject)
  private
    FParams: TStringList;
  public
  ...
  { TParseErrorRelated }
  TParseErrorRelated = record
    Range: TParseSourceRange;
    Message: string;
  end;
  ...
  TParseError = record
    Range: TParseSourceRange;
    Severity: TParseErrorSeverity;
    Code: string;
    Message: string;
    Related: TArray<TParseErrorRelated>;
  ...
  { TParseErrors }
  TParseErrors = class(TParseBaseObject)
  private
    FItems: TList<TParseError>;
    FMaxErrors: Integer;
  ...
  public
    ...
    procedure Add(
      const ARange: TParseSourceRange;
      const ASeverity: TParseErrorSeverity;
      const ACode: string;
    ...
    function GetItems(): TList<TParseError>;
  ...
  { TParseStatusObject }
  TParseStatusObject = class(TParseBaseObject)
  protected
    FStatusCallback: TParseCallback<TParseStatusCallback>;
  public
    constructor Create(); override;
    destructor Destroy(); override;
    procedure Status(const AText: string); overload;
    procedure Status(const AText: string; const AArgs: array of const); overload;
    function  GetStatusCallback(): TParseStatusCallback;
    procedure SetStatusCallback(const ACallback: TParseStatusCallback; const AUserData: Pointer = nil); virtual;
  end;

  { TParseErrorsObject }
  TParseErrorsObject = class(TParseStatusObject)
  protected
    FErrors: TParseErrors;
  public
    procedure SetErrors(const AErrors: TParseErrors); virtual;
    function GetErrors(): TParseErrors;
  end;

  { TParseOutputObject }
  TParseOutputObject = class(TParseStatusObject)
  protected
  ...
  public
    ...
    function GetOutputCallback(): TParseCaptureConsoleCallback;
  end;
...
function TParseCallback<T>.IsAssigned(): Boolean;
begin
...
class function TParseUtils.EnableVirtualTerminalProcessing(): Boolean;
var
  HOut: THandle;
  LMode: DWORD;
begin
...
class procedure TParseUtils.InitConsole();
begin
...
class procedure TParseUtils.FailIf(const Cond: Boolean; const Msg: string; const AArgs: array of const);
begin
  if Cond then
    raise Exception.CreateFmt(Msg, AArgs);
end;

class function TParseUtils.GetTickCount(): DWORD;
begin
  Result := WinApi.Windows.GetTickCount();
end;

class function TParseUtils.GetTickCount64(): UInt64;
begin
  Result := WinApi.Windows.GetTickCount64();
end;

class function TParseUtils.HasConsole(): Boolean;
begin
  Result := Boolean(GetConsoleWindow() <> 0);
end;

class procedure TParseUtils.ClearToEOL();
begin
  if not HasConsole() then Exit;
  Write(#27'[0K');
end;

class procedure TParseUtils.Print();
begin
  Print('');
end;

class procedure TParseUtils.PrintLn();
begin
  PrintLn('');
end;

class procedure TParseUtils.Print(const AText: string);
begin
  if not HasConsole() then Exit;
  Write(AText + COLOR_RESET);
end;

class procedure TParseUtils.Print(const AText: string; const AArgs: array of const);
begin
  if not HasConsole() then Exit;
  Write(Format(AText, AArgs) + COLOR_RESET);
end;

class procedure TParseUtils.PrintLn(const AText: string);
begin
  if not HasConsole() then Exit;
  WriteLn(AText + COLOR_RESET);
end;

class procedure TParseUtils.PrintLn(const AText: string; const AArgs: array of const);
begin
  if not HasConsole() then Exit;
  WriteLn(Format(AText, AArgs) + COLOR_RESET);
end;

class function TParseUtils.RGB(const AR, AG, AB: Byte): string;
begin
  Result := #27'[38;2;' + IntToStr(AR) + ';' + IntToStr(AG) + ';' + IntToStr(AB) + 'm';
end;

class function TParseUtils.Pause(const AMsg, AQuit: string): Boolean;
var
  LInput: string;
begin
  Result := False;
...
class function TParseUtils.AsUTF8(const AValue: string; ALength: PCardinal): Pointer;
begin
...
class function TParseUtils.ToAnsi(const AValue: string): AnsiString;
var
  LBytes: TBytes;
begin
...
class procedure TParseUtils.ProcessMessages();
var
  LMsg: TMsg;
begin
...
class function TParseUtils.RunPE(const AExe, AParams, AWorkDir: string; const AWait: Boolean; const AShowCmd: Word): Cardinal;
var
  LAppPath: string;
  LCmd: UnicodeString;
  LSI: STARTUPINFOW;
...
class function TParseUtils.WindowsPathToWSL(const APath: string): string;
var
  LFullPath: string;
  LDrive: Char;
begin
...
class function TParseUtils.RunElf(const AElf, AWorkDir: string): Cardinal;
var
  LWslPath: string;
  LCmd: UnicodeString;
  LSI: STARTUPINFOW;
...
class procedure TParseUtils.CaptureConsoleOutput(const ATitle: string; const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TParseCaptureConsoleCallback);
const
  CReadBuffer = 1024 * 2;
var
...
class procedure TParseUtils.CaptureZigConsolePTY(const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TParseCaptureConsoleCallback);
const
  CReadBuffer = 4096;
var
...
class function TParseUtils.CreateProcessWithPipes(const AExe, AParams, AWorkDir: string; out AStdinWrite: THandle; out AStdoutRead: THandle; out AProcessHandle: THandle; out AThreadHandle: THandle): Boolean;
var
...
class function TParseUtils.CreateDirInPath(const AFilename: string): Boolean;
var
  LPath: string;
begin
...
class function TParseUtils.GetZigExePath(): string;
var
  LBase: string;
begin
...
class function TParseUtils.GetExePath(): string;
begin
  Result := TPath.GetDirectoryName(ParamStr(0));
end;

class procedure TParseUtils.CopyFilePreservingEncoding(const ASourceFile, ADestFile: string);
var
  LSourceBytes: TBytes;
begin
...
class function TParseUtils.DetectFileEncoding(const AFilePath: string): TEncoding;
var
  LBytes: TBytes;
  LEncoding: TEncoding;
begin
...
class function TParseUtils.EnsureBOM(const AText: string): string;
const
  UTF16_BOM = #$FEFF;
begin
  Result := AText;
...
class function TParseUtils.EscapeString(const AText: string): string;
var
  LI: Integer;
  LChar: Char;
  LNextChar: Char;
...
class function TParseUtils.StripAnsi(const AText: string): string;
var
...
class function TParseUtils.ExtractAnsiCodes(const AText: string): string;
var
  LResult: TStringBuilder;
  LIdx: Integer;
  LLen: Integer;
...
class function TParseUtils.GetVersionInfo(out AVersionInfo: TParseVersionInfo; const AFilePath: string): Boolean;
var
  LFileName: string;
  LInfoSize: DWORD;
  LHandle: DWORD;
...
class function TParseUtils.IsValidWin64PE(const AFilePath: string): Boolean;
var
  LFile: TFileStream;
...
class procedure TParseUtils.UpdateIconResource(const AExeFilePath, AIconFilePath: string);
type
...
class procedure TParseUtils.UpdateVersionInfoResource(const PEFilePath: string; const AMajor, AMinor, APatch: Word; const AProductName, ADescription, AFilename, ACompanyName, ACopyright: string);
type
...
class function TParseUtils.ResourceExist(const AResName: string): Boolean;
begin
  Result := Boolean((FindResource(HInstance, PChar(AResName), RT_RCDATA) <> 0));
end;

class function TParseUtils.AddResManifestFromResource(const AResName: string; const AModuleFile: string; ALanguage: Integer): Boolean;
var
  LHandle: THandle;
...
class function TParseUtils.GetFileSHA256(const APath: string): string;
begin
  Result := THashSHA2.GetHashStringFromFile(APath).ToLower();
end;

class function TParseUtils.GetRelativePath(const ABasePath, AFullPath: string): string;
var
  LBasePath: string;
  LFullPath: string;
  LBaseLen: Integer;
...
class function TParseUtils.NormalizePath(const APath: string): string;
begin
  Result := APath.Replace(PathDelim, '/');
end;

class function TParseUtils.GetEnv(const AName: string): string;
begin
  Result := GetEnvironmentVariable(AName);
end;

class procedure TParseUtils.SetEnv(const AName: string; const AValue: string);
begin
  SetEnvironmentVariable(PChar(AName), PChar(AValue));
end;

class function TParseUtils.HasEnv(const AName: string): Boolean;
begin
  Result := not GetEnv(AName).IsEmpty();
end;

class function TParseUtils.RunFromIDE(): Boolean;
begin
  Result := HasEnv('BDS');
end;
...
constructor TParseBaseObject.Create();
begin
  inherited;
end;

destructor TParseBaseObject.Destroy();
begin
  inherited;
end;

function TParseBaseObject.Dump(const AId: Integer): string;
begin
  Result := '';
end;

procedure TParseBaseObject.InitConfig();
begin
end;

procedure TParseBaseObject.LoadConfig();
begin
end;

procedure TParseBaseObject.SaveConfig();
begin
end;

{ TParseCommandBuilder }

constructor TParseCommandBuilder.Create();
begin
  inherited;
...
destructor TParseCommandBuilder.Destroy();
begin
  FreeAndNil(FParams);
...
procedure TParseCommandBuilder.Clear();
begin
  FParams.Clear();
end;

procedure TParseCommandBuilder.AddParam(const AParam: string);
begin
  if AParam <> '' then
    FParams.Add(AParam);
end;

procedure TParseCommandBuilder.AddParam(const AFlag, AValue: string);
begin
  if AFlag <> '' then
  begin
    if AValue <> '' then
...
procedure TParseCommandBuilder.AddQuotedParam(const AFlag, AValue: string);
begin
  if AValue = '' then
    Exit;
...
procedure TParseCommandBuilder.AddQuotedParam(const AValue: string);
begin
  AddQuotedParam('', AValue);
end;

procedure TParseCommandBuilder.AddFlag(const AFlag: string);
begin
  if AFlag <> '' then
    FParams.Add(AFlag);
end;

function TParseCommandBuilder.Dump(const AId: Integer): string;
var
  LI: Integer;
begin
...
function TParseCommandBuilder.GetParamCount(): Integer;
begin
...
procedure TParseSourceRange.Clear();
begin
  Filename := '';
  StartLine := 0;
  StartColumn := 0;
...
function TParseSourceRange.IsEmpty(): Boolean;
begin
  Result := (StartLine = 0) and (StartColumn = 0);
end;

function TParseSourceRange.ToPointString(): string;
begin
  if IsEmpty() then
    Result := ''
  else
...
function TParseSourceRange.ToRangeString(): string;
begin
  if IsEmpty() then
    Result := ''
...
function TParseError.GetSeverityString(): string;
begin
  case Severity of
...
function TParseError.ToIDEString(): string;
begin
...
function TParseError.ToFullString(): string;
var
...
constructor TParseErrors.Create();
begin
  inherited;

  FItems := TList<TParseError>.Create();
  FMaxErrors := DEFAULT_MAX_ERRORS;
end;

destructor TParseErrors.Destroy();
begin
  FItems.Free();
...
function TParseErrors.CountErrors(): Integer;
var
  LError: TParseError;
begin
  Result := 0;
...
procedure TParseErrors.AddRelated(
  const ARange: TParseSourceRange;
  const AMessage: string
);
var
  LError: TParseError;
  LRelated: TParseErrorRelated;
  LLen: Integer;
begin
...
function TParseErrors.HasHints(): Boolean;
var
  LError: TParseError;
begin
  Result := False;
...
function TParseErrors.HasWarnings(): Boolean;
var
  LError: TParseError;
begin
  Result := False;
...
function TParseErrors.HasErrors(): Boolean;
var
  LError: TParseError;
begin
  Result := False;
...
function TParseErrors.HasFatal(): Boolean;
var
  LError: TParseError;
begin
  Result := False;
...
function TParseErrors.Count(): Integer;
begin
  Result := FItems.Count;
end;

function TParseErrors.ErrorCount(): Integer;
begin
  Result := CountErrors();
end;

function TParseErrors.WarningCount(): Integer;
var
  LError: TParseError;
begin
  Result := 0;
...
function TParseErrors.ReachedMaxErrors(): Boolean;
begin
  Result := CountErrors() >= FMaxErrors;
end;

procedure TParseErrors.Clear();
begin
  FItems.Clear();
end;

function TParseErrors.GetItems(): TList<TParseError>;
begin
  Result := FItems;
end;

function TParseErrors.GetMaxErrors(): Integer;
begin
  Result := FMaxErrors;
end;

procedure TParseErrors.SetMaxErrors(const AMaxErrors: Integer);
begin
  FMaxErrors := AMaxErrors;
end;

function TParseErrors.Dump(const AId: Integer): string;
var
  LBuilder: TStringBuilder;
  LI: Integer;
begin
...
constructor TParseStatusObject.Create();
begin
  inherited;
end;

destructor TParseStatusObject.Destroy();
begin
  inherited;
end;

procedure TParseStatusObject.Status(const AText: string);
begin
  if FStatusCallback.IsAssigned() then
    FStatusCallback.Callback(AText, FStatusCallback.UserData);
end;

procedure TParseStatusObject.Status(const AText: string; const AArgs: array of const);
begin
  Status(Format(AText, AArgs));
end;

function TParseStatusObject.GetStatusCallback(): TParseStatusCallback;
begin
  Result := FStatusCallback.Callback;
end;

procedure TParseStatusObject.SetStatusCallback(const ACallback: TParseStatusCallback; const AUserData: Pointer);
begin
...
procedure TParseErrorsObject.SetErrors(const AErrors: TParseErrors);
begin
  FErrors := AErrors;
end;

function TParseErrorsObject.GetErrors(): TParseErrors;
begin
  Result := FErrors;
end;
...
procedure TParseOutputObject.SetOutputCallback(
...
function TParseOutputObject.GetOutputCallback(): TParseCaptureConsoleCallback;
begin
